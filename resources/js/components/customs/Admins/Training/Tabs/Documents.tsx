...
import type { DocsData } from "@/Pages/App/Admin/Training";
...
const InputBuilder = ({ name, callback, className, value, file_type, getErr }: { name: string, className?: string, callback?: (name: string, value: FileData) => void, value?: FileData, file_type: string[], getErr: (hasErr: boolean) => void }) => {
...
const PostData = async (data: any) => {
    const res = await axios.post("/api/admin/training/phase3", data);
    return res.data;
}

const checkDataChange = (current: FileData[], stateData: DocsData[]) => {
    let isContentChanged: boolean[] = []

    if (current.length !== stateData.length) return true

    stateData.map((item: DocsData, index: number) => {
        ...
        if (contentValue !== item.files || typeValue !== item.file_type) isContentChanged[index] = true
    })
...
export default function Documents({
    ...
    NextButton: () => void,
    PrevButton: () => void,
    obtainValue: (value: DocsData[]) => void,
    fileData: DocsData[],
}) {
    ...
    const {
        ...
        fileData.map((item: DocsData) => item.url),
        "docsData",
        fileData.map((item: DocsData) => item.files)
    )

    if (errorAssets) {
        axios.delete(`/api/cookie/admin-training-phase-3`).then(() => {
            toast({
                title: "Error",
                description: "Tidak dapat menemukan file yang dibutuhkan",
                variant: "destructive",
                duration: 5000,
            });
    ...
    useEffect(() => {
        ...
        const preparedAsset = assets.map((item: File, index: number) => {
            const targetFile = fileData.find((val: DocsData) => val.files === item.name)

            if (fileData.length <= assets.length && targetFile)
                return { content: item, type: targetFile.file_type }
        })

        if (preparedAsset.some((item: FileData | undefined) => !item)) return;
        setFile(preparedAsset)
    }, [assets])

    const handleNext = async () => {
        const isDataChanged = checkDataChange(file, fileData)
