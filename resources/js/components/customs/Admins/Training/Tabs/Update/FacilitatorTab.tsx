...
import { PostDataTrainingStaff, UserSelectedTableBuilder, UserSelectorBuilder, type FileUpload, type UserType } from "@/components/customs/Admins/Training/TraininMembersFile"
...
export default function FacilitatorTab({
    ...
    const handleNext = async () => {
        ...
        const result = await PostDataTrainingStaff(formData, 5);
        if (result.success) {
            toast({
                title: "Berhasil",
            ...
            const preservedResult: StaffData[] = result.data.facilitator.map((item: any, index: number) => {
    ...
    return (
        <TabsContent value="fasilitator" className="w-full px-3 py-1">
            <h4 className="text-lg font-bold">Fasilitator</h4>
            <div className="flex flex-col gap-3">
                <UserSelectorBuilder selectedTargets={selected} trainingTargets={facilitator} callback={handleTargetChange} />
                {
                    selected.length > 0 &&
