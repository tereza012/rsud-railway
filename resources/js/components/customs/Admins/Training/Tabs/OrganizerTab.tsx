...
const PostData = async (formData: FormData) => {
    const data = await axios.post("/api/admin/training/phase4", formData)
...
export default function OrganizerTab({
    ...
    useEffect(() => {
        ...
        organizersData.map((item: StaffData) => {
        ...
        organizersData.map(async (item: StaffData) => {
            const assets = await callAssets(
                item.files.map((file: DocsData) => file.url),
                item.files.map((file: DocsData) => file.files)
            )
    ...
    const {
        ...
        organizersData.flatMap((item: StaffData) => item.files.map((val: DocsData) => val.url)),
        "docsData",
        organizersData.flatMap((item: StaffData) => item.files.map((val: DocsData) => val.files))
    )
    ...
    useEffect(() => {
        ...
        const preparedFiles = assets.map((item: File) => {
            const userData = organizersData.find((content: StaffData) => content.files.find((file: DocsData) => file.files === item.name))
            if (!userData) return
