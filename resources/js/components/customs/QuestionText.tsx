...
const QuestionText = ({
    ...
    const [input, setInput] = useState<InputData[]>(
        content?.length > 0
            ? content?.map(({ question, answer }: QuestionType, index: number) => {
                return { name: name, value: question, answer: answer };
            })
            : [{ name, value: "", answer: '' }]
    );
    ...
    useEffect(() => {
        if (content.length > 0)
            setInput(
                content.map(({ question, answer }: QuestionType) => {
                    return { name: name, value: question, answer: answer };
                })
            );
